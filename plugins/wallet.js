...
export default (ctx, inject) => {
    ...
    const wallet = Vue.observable({
        ...
        async connect() {
            ...
            const isMetamask = instance.isMetaMask || instance.walletMeta?.name === 'MetaMask'
            if(isMetamask) {
        ...
        async switchNetwork(chainId) {
            ...
            if(!this.type === WALLET_TYPE.Metamask) {
                ...
                throw new Error("Selected wallet/account doesn't support network switching. Please use Desktop Metamask instead.")
            }
			...
			try {
                ...
                if(
                    err?.message === 'JSON RPC response format is invalid'
                ) return
