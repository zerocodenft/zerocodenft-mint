<template>
	<div class="text-center">
		...
		>
		<TweetModal :images="mintedTokens" :mintCount="mintCount" @hidden="handleTweetModalHide"></TweetModal>
	</div>
</template>
	...
	methods: {
		...
		async mint() {
			...
			try {
				...
				const tokenURIPromises = events.map(e=>signedContract.tokenURI(e.args.tokenId))
				const tokenURIs = await Promise.all(tokenURIPromises);

				const metadataPromises = tokenURIs.map((uri) =>fetch('https://ipfs.io/ipfs/' + uri.replace('ipfs://', '')))
